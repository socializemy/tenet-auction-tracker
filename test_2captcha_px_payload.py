...
async def solve_px(api_key, page_url, app_id, vid, uuid, host_url):
    ...
    async with async_playwright() as p:
        ...
        if "px-captcha" in content:
            ...
            try:
                ...
                vid = ""
                if "window._pxVid = '" in content:
                    vid = content.split("window._pxVid = '")[1].split("'")[0]
                    
                uuid = ""
                if "window._pxUuid = '" in content:
                    uuid = content.split("window._pxUuid = '")[1].split("'")[0]
                    
                host_url = ""
                if "window._pxHostUrl = '" in content:
                    host_url = content.split("window._pxHostUrl = '")[1].split("'")[0]
                
                print(f"Extracted Params: AppId={app_id}, Vid={vid}, Uuid={uuid}, HostUrl={host_url}")
                
                token = await solve_px(API_KEY, target_url, app_id, vid, uuid, host_url)
